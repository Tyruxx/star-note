  ...
  try {
    ...
    if (query.base != '' && query.currency != '') {
      for (const date of query.dates) {
        try {
          const result = await $fetch<Result>(`https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@${date}/v1/currencies/${query.base?.toLowerCase() ?? ''}.json`)
      ...
      for (const currency of Object.values(currencyCollection)) {
        const result = await $fetch<Result>(`https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@${query.dates}/v1/currencies/${currency[0].toLowerCase()}.json`)
